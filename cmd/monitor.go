...
var monitorCmd = &cobra.Command{
	Use:   "monitor [type] [source]",
	Short: "Unified monitoring command",
	Long:  "Monitor various sources (file, command) and send notifications",
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		// Create monitor source based on type
		var monitorSource collector.MonitorSource
		var commandArgs []string

		switch monitorType {
		case "file":
			monitorSource = collector.NewFileSource(source)
		case "command":
...
func init() {
	...
	// Add command line parameters (same as start and exec commands)
	monitorCmd.Flags().IntP("line-threshold", "l", 0, "Number of lines to trigger summary (overrides global config)")
	monitorCmd.Flags().StringP("interval", "i", "", "Check interval (e.g., 30s, 1m) (overrides global config)")
	monitorCmd.Flags().StringP("chat-id", "c", "", "Telegram chat ID (overrides global config)")
	monitorCmd.Flags().BoolP("daemon", "d", false, "Run in daemon mode (background)")
	monitorCmd.Flags().StringP("name", "n", "", "Custom process name (used in daemon mode)")
	monitorCmd.Flags().StringP("workdir", "w", "", "Working directory for command execution")
	monitorCmd.Flags().Bool("final-summary", false, "Enable final summary on program exit (overrides global config)")
	monitorCmd.Flags().Bool("no-final-summary", false, "Disable final summary on program exit")
	monitorCmd.Flags().BoolP("error-only", "E", false, "Only send notifications for errors and exceptions")
	monitorCmd.Flags().BoolP("final-summary-only", "F", false, "Only send notifications for final summary")
	monitorCmd.Flags().StringSlice("notifiers", []string{}, "Enable specific notifiers (comma-separated: telegram,email)")
}

func runUnifiedMonitor(monitorSource collector.MonitorSource, lineThreshold *int, checkInterval *time.Duration, chatID *string, workingDir string, finalSummary *bool, errorOnlyMode *bool, finalSummaryOnly *bool, enabledNotifiers []string) error {
...
func runUnifiedDaemon(monitorSource collector.MonitorSource, lineThreshold *int, checkInterval *time.Duration, chatID *string, processName string, workingDir string, finalSummary *bool, errorOnlyMode *bool, finalSummaryOnly *bool, enabledNotifiers []string) error {
	...
	sourceIdentifier := monitorSource.GetIdentifier()
	if processName != "" {
		...
		processID = manager.GenerateProcessID(sourceIdentifier)
	}
	...
	return runUnifiedMonitor(monitorSource, lineThreshold, checkInterval, chatID, workingDir, finalSummary, errorOnlyMode, finalSummaryOnly, enabledNotifiers)
}
